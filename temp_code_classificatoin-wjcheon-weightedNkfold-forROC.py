    ...
    for iter1 in range(1, 6):
        ...
        for e in tqdm(range(1, EPOCHS + 1)):
            ...
            for X_train_batch, y_train_batch in train_loader:
                ...
                #train_loss = criterion(y_train_pred, y_train_batch.unsqueeze(1).long())
                train_loss = criterion(y_train_pred.squeeze(), y_train_batch.squeeze().float())
                # Accuracy
                train_acc = np.average(np.equal(y_train_pred.reshape(-1).detach().cpu().numpy().round(), y_train_batch.cpu()))


                #train_acc = multi_acc(y_train_pred, y_train_batch)
                #acc = binary_acc(y_train_pred, y_train_batch.unsqueeze(1))
                optimizer.zero_grad()
                train_loss.backward()
                optimizer.step()
        ...
        # VALIDATION
            ...
            print(f'Fold {iter1} | Epoch {e + 0:03}: | Train Loss: {train_epoch_loss / len(train_loader):.5f} | Val Loss: {val_epoch_loss / len(val_loader):.5f} |'
                  f' Train Acc: {train_epoch_acc / len(train_loader):.3f}| Val Acc: {val_epoch_acc / len(val_loader):.3f}')
...
#            ws_write.cell(row=i + 1, column=6).value = preSoftmaxSetF_list[i][0][1]
    ...
    saveFileName = os.path.join(r"D:\BladderToxicityResult_balanced_148_age_auroc", currentDT.strftime("%Y-%m-%d-%H-%M-%S-Results.xlsx"))
